 ...
 */
public class ChineseNumWordsRulesClassifier implements BoilerpipeFilter {
    public static final ChineseNumWordsRulesClassifier INSTANCE = new ChineseNumWordsRulesClassifier();

    /**
     * Returns the singleton instance for RulebasedBoilerpipeClassifier.
     */
    public static ChineseNumWordsRulesClassifier getInstance() {
        return INSTANCE;
    }
    ...
    protected boolean classify(final TextBlock prev, final TextBlock curr,
        ...
        if (curr.getLinkDensity() <= 0.333333) {
            if (prev.getLinkDensity() <= 0.555556) {
                ...
                if (curr.getNumWords() <= 120) {
                    if (next.getNumWords() <= 178) {
                        isContent = false;
                    } else {
                        isContent = true;
                    }
