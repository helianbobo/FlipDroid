...
import com.goal98.girl.util.TextPaintUtil;
...
public class ContentPagerAdapter extends PagerAdapter {
    ...
    private int index = 0;
    private List<WebImageView> images = new ArrayList<WebImageView>();
    private Map<Integer, List<WebImageView>> maps = new HashMap<Integer, List<WebImageView>>();

    public List<WebImageView> getItem(int i){
        return maps.get(i);
    }
    ...
    private void doPage(List<TikaUIObject> paragraphsList) {
        ...
        final LinearLayout.LayoutParams adLayoutParams = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT);
...
//                    clonedTextView.setMovementMethod(LinkMovementMethod.getInstance());
            ...
            if (uiObject.getType().equals(TikaUIObject.TYPE_IMAGE)) {
                ...
                if (imageInfo.getWidth() == 0)
                    imageInfo.setWidth((int) (deviceInfo.getWidth() - 30 * deviceInfo.getDensity()));

                if (imageInfo.getHeight() == 0)
                    imageInfo.setHeight((int) (maxHeightInPixel+30));
...
//        if(contentHolderView.getChildCount()>1)
    ...
    private LinearLayout resetNewPage(float maxHeightInPixel, LinearLayout layout) {
        maps.put(index,images);
        images = new ArrayList<WebImageView>();
        index++;
