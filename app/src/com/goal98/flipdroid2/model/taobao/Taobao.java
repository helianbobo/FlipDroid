package com.goal98.flipdroid2.model.taobao;

public class Taobao {
    public static String TAOBAOURI="http://gw.api.taobao.com/router/rest";
    public static String TESTTAOBAOURI="http://gw.api.tbsandbox.com/router/rest";
    public static String TAOBAOGETTIEMSURI="http://gw.api.taobao.com/router/rest";
    public static String APPKEY="12307981";
    public static String APPSERECT="7961d6948156f1350d4ba779087b0f1f";
    public static final String TYPE_TAOBAO="TAOBAO";
    public static final String TTID="400000_"+APPKEY+"@ter_android_1.00";
    public static final String ITEMURL="http://item.taobao.com/item.htm?id=";
    public static final String ITEMWAPURLFORMAT="http://auction1.wap.taobao.com/auction/item_detail-0db0-%s.jhtml";
    


}
